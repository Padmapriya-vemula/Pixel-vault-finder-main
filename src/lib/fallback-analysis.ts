...
async function getBuffer(imageData: Uint8Array | File): Promise<ArrayBuffer> {
  if (imageData instanceof File) {
    return await imageData.arrayBuffer();
  }
  return imageData.buffer instanceof ArrayBuffer 
    ? imageData.buffer 
...
export async function fallbackAnalysis(imageData: Uint8Array | File): Promise<ImageAnalysis> {
  ...
  return {
    ...
    tags: Array.from(new Set(tags)).slice(0, 15) // Remove duplicates and limit to 15 tags
  };
}